export default class BurgerMenu {
  ...
  onBurgerClick() {
    ...
    if (isOpen) {
      const firstMenuLink = this.burgerMenu.querySelector(`.${this.config.MENU_LINK}`)
  ...
  hideBurgerMenu(returnFocus = false) {
  ...
  getFocusableElements() {
    return [
      this.burgerButton,
      ...this.burgerMenu.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'),
    ]
  }
  ...
  onBodyClick(event) {
    ...
    const isLinkInMenu = Boolean(target.closest(`.${this.config.HEADER_MENU} a[href]`))
    ...
    if ((isLinkInMenu && window.innerWidth <= this.config.BREAKPOINT) || (isMenuOpen && isClickOutsideMenu)) {
      this.hideBurgerMenu()
    }
  }
